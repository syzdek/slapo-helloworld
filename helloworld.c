...
#define HELLO_CFG_EXAMPLE_STR		0x04
...
#define HELLO_DFLT_EXAMPLE_STR		"Hello World"
...
static ConfigTable hello_cf_ats[] =
{
	...
	{	.name		= "hello_example_dn_attr",
		.what		= "<attribute>",
		.min_args	= 2,
		.max_args	= 2,
		.length		= 0,
		.arg_type	= ARG_ATDESC|ARG_MAGIC|HELLO_CFG_EXAMPLE_DNATTR,
		.arg_item	= hello_cf_gen,
		.attribute	= "( 1.3.6.1.4.1.27893.4.3.4.102"
					" NAME 'olcHelloExampleDnAttr'"
					" DESC 'Example processing of attribute in slapd.conf'"
					" EQUALITY caseIgnoreMatch"
	...
	{	.name		= "hello_example_str",
		.what		= "<string>",
		.min_args	= 2,
		.max_args	= 2,
		.length		= 0,
		.arg_type	= ARG_STRING|ARG_MAGIC|HELLO_CFG_EXAMPLE_STR,
		.arg_item	= hello_cf_gen,
		.attribute	= "( 1.3.6.1.4.1.27893.4.3.4.104"
					" NAME 'olcHelloExampleStr'"
					" DESC 'Example processing of string in slapd.conf'"
					" EQUALITY caseIgnoreMatch"
					" ORDERING caseIgnoreOrderingMatch"
					" SUBSTR caseIgnoreSubstringsMatch"
					" SYNTAX OMsDirectoryString"
		...
		ConfigArgs *				c )
{
	...
	switch (c->op)
	{
		...
		switch(c->type)
		{
			case HELLO_CFG_EXAMPLE_DNATTR:
			c->value_ad = hw->hw_example_dn_attr;
			return(0);
			...
			if ((rc = value_add_one(&c->rvalue_vals, &bv)) != 0)
				return(rc);
			return(value_add_one(&c->rvalue_nvals, &bv));

			case HELLO_CFG_EXAMPLE_STR:
			if ( hw->hw_example_str != NULL)
			{
				bv.bv_val = hw->hw_example_str;
				bv.bv_len = strlen(bv.bv_val);
				if ((rc = value_add_one(&c->rvalue_vals, &bv)) != 0)
					return(rc);
				return(value_add_one(&c->rvalue_nvals, &bv));
		...
		switch(c->type)
		{
			...
			case HELLO_CFG_EXAMPLE_STR:
			if (hw->hw_example_str != NULL)
				ber_memfree(hw->hw_example_str);
			hw->hw_example_str = ch_strdup(HELLO_DFLT_EXAMPLE_STR);
			return(0);
		...
		switch( c->type )
		{
			...
			if (!(is_at_syntax(ad->ad_type, SLAPD_DN_SYNTAX)))
			{
				snprintf( c->cr_msg,
					sizeof( c->cr_msg ),
					"hello_example_dn_attr attribute=\"%s\" must have DN (%s) syntax",
					c->argv[1],
					SLAPD_DN_SYNTAX );
				Debug(LDAP_DEBUG_CONFIG, "%s: %s.\n", c->log, c->cr_msg);
				return(ARG_BAD_CONF);
			};
			hw->hw_example_dn_attr = ad;
			return(0);
			...
			case HELLO_CFG_EXAMPLE_STR:
			Debug(LDAP_DEBUG_TRACE, "==> hello_cf_gen add EXAMPLE_STR\n");
			if (hw->hw_example_str != NULL)
				ch_free(hw->hw_example_str);
			hw->hw_example_str = c->value_string;
			c->value_string = NULL;
			return(0);
		...
		ConfigReply *				cr )
{
	...
	Debug(LDAP_DEBUG_CONFIG, "hello_example_str:     %s\n", hw->hw_example_str);
